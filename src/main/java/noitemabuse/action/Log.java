...
public class Log extends Action {
    ...
    public String getMessage(Player player, ItemStack item, Event event, String eventMessage) {
        ...
        if(durability != 0) reasons.add(Message.format(player, Message.REASON_OVERDURABLE, "$durability:" + durability));
        if(enchant != null) reasons.add(Message.format(player, Message.REASON_OVERENCHANT, "$enchant:" + enchantName, "$level:" + level, "$max:" + max));
