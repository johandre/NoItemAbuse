...
public class NBTFactory {
    ...
    private Method loadCompund;
    private Method saveCompund;
    ...
    public final class NBTCompound extends ConvertedMap {
         ...
         * @param compression - whether or not to compress the output.
         * @throws IOException If anything went wrong.
         */
        public void saveTo(OutputSupplier<? extends OutputStream> stream, boolean compression) throws IOException {
            saveStream(this, stream, compression);
        }
    ...
    private NBTFactory() {
        if (baseClass == null) {
            try {
                ...
                // Loading/saving
                streamTools = loader.loadClass(baseClass.getPackage().getName() + ".NBTCompressedStreamTools");
                loadCompund = getMethod(Modifier.STATIC, 0, streamTools, null, DataInput.class);
                saveCompund = getMethod(Modifier.STATIC, 0, streamTools, null, baseClass, DataOutput.class);
    ...
    private String getPackageName() {
        ...
        // server != null ? server.getClass().getPackage().getName() : null;
        // if (name != null && name.contains("craftbukkit"))
        return name; // it's not null, and we don't have a choice. take it like it is, this is a Bukkit plugin after all
    }
     ...
     * @throws IOException If anything went wrong.
     */
    public static NBTCompound fromStream(InputSupplier<? extends InputStream> stream, boolean compression) throws IOException {
        InputStream input = null;
        DataInputStream data = null;
        try {
            input = stream.getInput();
            data = new DataInputStream(new BufferedInputStream(compression ? new GZIPInputStream(input) : input));
            return fromCompound(invokeMethod(get().loadCompund, null, data));
        } finally {
            if (data != null) {
     ...
     */
    public static void saveStream(NBTCompound source, OutputSupplier<? extends OutputStream> stream, boolean compression) throws IOException {
        OutputStream output = null;
        DataOutputStream data = null;
        try {
            output = stream.getOutput();
            data = new DataOutputStream(compression ? new GZIPOutputStream(output) : output);
            invokeMethod(get().saveCompund, null, source.getHandle(), data);
        } finally {
            if (data != null) {
                Closeables.closeQuietly(data);
            }
